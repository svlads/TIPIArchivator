...
void PrefixTree::BuildTree() {
  ...
  while (iteration_count--) {
    ...
    if (minsum == sum_from_nodes) {
      ...
      left = new TNode(values_[start].value, nullptr, nullptr);
      left->symbol = values_[start].symbol;
      right = new TNode(values_[start + 1].value, nullptr, nullptr);
    ...
    next_node = new TNode(left->value + right->value, left, right);
    sums.Push(next_node);
  }
  root_ = sums.Front();
}
...
void PrefixTree::FindCodes(TNode *node, Code cur_value) {
  if (node->left == nullptr && node->right == nullptr) {
    ...
    cur_value.Append(0);
    cur_first.Append(1);
    FindCodes(node->left, cur_value);
    FindCodes(node->right, cur_first);
  }
}
...
void PrefixTree::WriteTree(Code &code, unsigned char *symbols_order) {
  // Code code(2 * 256);
  size_t child = 0;
  WriteTree(root_, root_, code, symbols_order, child);
}
...
void PrefixTree::WriteTree(PrefixTree::TNode *node, PrefixTree::TNode *parent, Code &code, unsigned char *symbols_order,
  ...
  if (node->left != nullptr && node->right != nullptr) {
    code.Append(1);
    WriteTree(node->left, node, code, symbols_order, child);
    code.Append(0);
    WriteTree(node->right, node, code, symbols_order, child);
  } else {
